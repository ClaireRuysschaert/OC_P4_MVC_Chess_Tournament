...
def assign_player_pairs(tournament_id: int, current_round_number: int) -> list[list[str]]:
    ...
    if current_round_number == 0:
        ...
        for player in range(0, len(players), 2):
            ...
            Player.update_met_competitors_list_in_db(players[player], players[player+1])
            Player.update_met_competitors_list_in_db(players[player+1], players[player])
        return player_pairs
    else:
